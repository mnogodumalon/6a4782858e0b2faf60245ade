    ...
    while ((km = kpiRe.exec(src)) !== null) {
      ...
      for (const f of km[0].matchAll(/fields\.(\w+)/g)) {
        if (dimFields.has(f[1])) {
          warnings.push(`Field '${f[1]}' drives BOTH a clickable KPI and a ChartWidget dimension — the KPI is the decorated mirror of a chart segment; drop the KPI (the chart's tone carries that state).`);
        }
      }
    }
  }
