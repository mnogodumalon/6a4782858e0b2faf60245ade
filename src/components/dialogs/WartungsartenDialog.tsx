...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Wartungsarten';
...
interface WartungsartenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Wartungsarten['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Wartungsarten['fields'];
...
export function WartungsartenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: WartungsartenDialogProps) {
  const [fields, setFields] = useState<Partial<Wartungsarten['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Wartungsarten['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'wartungsarten');
      await onSubmit(clean as Wartungsarten['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Wartungsarten['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Wartungsarten bearbeiten' : 'Wartungsarten hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Beschreibung, Umfang, Besonderheiten..."
    ...
    'intervall_tage': (
      <div key="intervall_tage" className="space-y-1.5">
        <Label htmlFor="intervall_tage">Wartungsintervall (in Tagen)</Label>
        <Input
          id="intervall_tage"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'intervall_tage')}
          placeholder="z. B. 30, 60, 365"
          value={fields.intervall_tage !== undefined ? fields.intervall_tage : (computedValues['intervall_tage'] ?? '')}
          onChange={e => setFields(f => ({ ...f, intervall_tage: clampNumberValue(formEnhancements, 'intervall_tage', e.target.value) }))}
        />
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"bezeichnung": "Bezeichnung", "beschreibung": "Beschreibung", "intervall_tage": "Wartungsintervall (in Tagen)"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.WARTUNGSARTEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
