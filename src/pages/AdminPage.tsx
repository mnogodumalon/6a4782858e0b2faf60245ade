...
import { WartungsartenViewDialog } from '@/components/dialogs/WartungsartenViewDialog';
import { WartungsprotokolleDialog } from '@/components/dialogs/WartungsprotokolleDialog';
import { WartungsprotokolleViewDialog } from '@/components/dialogs/WartungsprotokolleViewDialog';
...
const WARTUNGSARTEN_FIELDS = [
  { key: 'bezeichnung', label: 'Bezeichnung', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'intervall_tage', label: 'Wartungsintervall (in Tagen)', type: 'number' },
];
const WARTUNGSPROTOKOLLE_FIELDS = [
  { key: 'titel', label: 'Titel / Bezeichnung', type: 'string/text' },
  { key: 'wartungsart', label: 'Wartungsart', type: 'applookup/select', targetEntity: 'wartungsarten', targetAppId: 'WARTUNGSARTEN', displayField: 'bezeichnung' },
  { key: 'datum_wartung', label: 'Datum der Wartung', type: 'date/date' },
  { key: 'naechste_wartung', label: 'Nächste Wartung', type: 'date/date' },
  { key: 'kosten', label: 'Kosten (€)', type: 'number' },
  { key: 'techniker', label: 'Durchgeführt von', type: 'string/text' },
...
const ENTITY_TABS = [
  { key: 'wartungsarten', label: 'Wartungsarten', pascal: 'Wartungsarten' },
  { key: 'wartungsprotokolle', label: 'Wartungsprotokolle', pascal: 'Wartungsprotokolle' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('wartungsarten');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'wartungsarten': new Set(),
    'wartungsprotokolle': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'wartungsarten': return (data as any).wartungsarten as Wartungsarten[] ?? [];
      case 'wartungsprotokolle': return (data as any).wartungsprotokolle as Wartungsprotokolle[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'wartungsprotokolle':
        lists.wartungsartenList = (data as any).wartungsarten ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'wartungsprotokolle' && fieldKey === 'wartungsart') {
      const match = (lists.wartungsartenList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'wartungsarten': return WARTUNGSARTEN_FIELDS;
      case 'wartungsprotokolle': return WARTUNGSPROTOKOLLE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'wartungsarten': return {
        create: (fields: any) => LivingAppsService.createWartungsartenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateWartungsartenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteWartungsartenEntry(id),
      };
      case 'wartungsprotokolle': return {
        create: (fields: any) => LivingAppsService.createWartungsprotokolleEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateWartungsprotokolleEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteWartungsprotokolleEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'wartungsarten' || dialogState?.entity === 'wartungsarten') && (
        <WartungsartenDialog
          open={createEntity === 'wartungsarten' || dialogState?.entity === 'wartungsarten'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'wartungsarten' ? handleUpdate : (fields: any) => handleCreate('wartungsarten', fields)}
          defaultValues={dialogState?.entity === 'wartungsarten' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Wartungsarten']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Wartungsarten']}
        />
      )}
      {(createEntity === 'wartungsprotokolle' || dialogState?.entity === 'wartungsprotokolle') && (
        <WartungsprotokolleDialog
          open={createEntity === 'wartungsprotokolle' || dialogState?.entity === 'wartungsprotokolle'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'wartungsprotokolle' ? handleUpdate : (fields: any) => handleCreate('wartungsprotokolle', fields)}
          defaultValues={dialogState?.entity === 'wartungsprotokolle' ? dialogState.record?.fields : undefined}
          wartungsartenList={(data as any).wartungsarten ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Wartungsprotokolle']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Wartungsprotokolle']}
        />
      )}
      {viewState?.entity === 'wartungsarten' && (
        <WartungsartenViewDialog
          open={viewState?.entity === 'wartungsarten'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'wartungsarten', record: r }); }}
        />
      )}
      {viewState?.entity === 'wartungsprotokolle' && (
        <WartungsprotokolleViewDialog
          open={viewState?.entity === 'wartungsprotokolle'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'wartungsprotokolle', record: r }); }}
          wartungsartenList={(data as any).wartungsarten ?? []}
        />
      )}
