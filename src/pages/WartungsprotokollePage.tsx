...
export default function WartungsprotokollePage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<Wartungsprotokolle[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Wartungsprotokolle | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Wartungsprotokolle | null>(null);
  ...
  async function handleCreate(fields: Wartungsprotokolle['fields']) {
  ...
  async function handleUpdate(fields: Wartungsprotokolle['fields']) {
    ...
    await LivingAppsService.updateWartungsprotokolleEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteWartungsprotokolleEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Wartungsprotokolle im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Wartungsprotokolle suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('techniker')}>
                <span className="inline-flex items-center gap-1">
                  Durchgeführt von
                  {sortKey === 'techniker' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/wartungsprotokolle/${record.record_id}`); }}>
                <TableCell className="font-medium">{record.fields.titel ?? '—'}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getWartungsartenDisplayName(record.fields.wartungsart)}</span></TableCell>
                ...
                <TableCell>{record.fields.techniker ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Wartungsprotokolle. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
