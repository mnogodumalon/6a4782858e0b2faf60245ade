...
import type { Wartungsarten, Wartungsprotokolle, CreateWartungsarten, CreateWartungsprotokolle } from '@/types/app';
...
export class LivingAppsService {
  // --- WARTUNGSARTEN ---
  static async getWartungsarten(): Promise<Wartungsarten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGSARTEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Wartungsarten[];
    return enrichLookupFields(records, 'wartungsarten');
  }
  static async getWartungsartenEntry(id: string): Promise<Wartungsarten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGSARTEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Wartungsarten;
    return enrichLookupFields([record], 'wartungsarten')[0];
  }
  static async createWartungsartenEntry(fields: CreateWartungsarten) {
    return callApi('POST', `/apps/${APP_IDS.WARTUNGSARTEN}/records`, { fields: cleanFieldsForApi(fields as any, 'wartungsarten') });
  }
  static async updateWartungsartenEntry(id: string, fields: Partial<CreateWartungsarten>) {
    return callApi('PATCH', `/apps/${APP_IDS.WARTUNGSARTEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'wartungsarten') });
  }
  static async deleteWartungsartenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WARTUNGSARTEN}/records/${id}`);
  }

  // --- WARTUNGSPROTOKOLLE ---
  static async getWartungsprotokolle(): Promise<Wartungsprotokolle[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGSPROTOKOLLE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Wartungsprotokolle[];
    return enrichLookupFields(records, 'wartungsprotokolle');
  }
  static async getWartungsprotokolleEntry(id: string): Promise<Wartungsprotokolle | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGSPROTOKOLLE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Wartungsprotokolle;
    return enrichLookupFields([record], 'wartungsprotokolle')[0];
  }
  static async createWartungsprotokolleEntry(fields: CreateWartungsprotokolle) {
    return callApi('POST', `/apps/${APP_IDS.WARTUNGSPROTOKOLLE}/records`, { fields: cleanFieldsForApi(fields as any, 'wartungsprotokolle') });
  }
  static async updateWartungsprotokolleEntry(id: string, fields: Partial<CreateWartungsprotokolle>) {
    return callApi('PATCH', `/apps/${APP_IDS.WARTUNGSPROTOKOLLE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'wartungsprotokolle') });
  }
  static async deleteWartungsprotokolleEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WARTUNGSPROTOKOLLE}/records/${id}`);
  }
